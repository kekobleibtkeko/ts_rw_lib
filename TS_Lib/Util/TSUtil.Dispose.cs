...
public static partial class TSUtil
{
	...
	{
		public ScrollPosition() : this(default) { }
	...
	public class EditBuffer_D<K, V>(K key, Func<V> def_func) : TSDisposableHelper<V>()
	{
		private static readonly Dictionary<K, V> Buffers = [];
		public V Ref = def_func();
		private readonly K Key = key;

		public override V GetValue()
		{
			return Buffers.Ensure(Key, () => def_func());
		}

		public override void SetValue(V value)
		{
			Ref = value;
		}
